...
#LABEL E ENTRADA DE DADOS DA TELA DE LOGIN
    ...
    def verificar_login(self):
        ...
        if usuario == "admin" and senha == "caio123":
    ...
    def editar_produto(self):
        ...
        def salvar():
            try:
                ...
                preco = float(entrada_preco.get().replace(",","."))
                cursor = self.conn.cursor()
                # CORREÇÃO: Adicionada a vírgula entre o SQL e os parâmetros
                ...
                print(e) # Imprime o erro no console para debug
        ...
        try: 
            ...
            if tipo == "entrada":
                ...
                if qtd > atual:
                    ...
                    return
                # CORREÇÃO: SQL de subtração correto
                cursor.execute("UPDATE produtos SET quantidade = quantidade - ? WHERE id=?", (qtd, id))
            
            # CORREÇÃO: Commit e atualização movidos para fora do if/else para funcionar em ambos
            self.conn.commit()
            self.atualizar_tabela()
